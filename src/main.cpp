...
struct SOIPDTModel
{
    ...
    {
        d = (int)roundf(L / dt);
        if (d >= MAX_DELAY_SAMPLES) d = MAX_DELAY_SAMPLES - 1;
...
struct ReferenceFilter
{
    ...
    struct FilterOutput { float val, dot, ddot; };
...
void driveActuator(float pwm_val)
{
    ...
    if (pwm_val > 0.0f) { pwmIN1.writeScaled(pwm_val); pwmIN2.writeScaled(0.0f); }
    else if (pwm_val < 0.0f) { pwmIN1.writeScaled(0.0f); pwmIN2.writeScaled(abs(pwm_val)); }
    else { pwmIN1.writeScaled(0.0f); pwmIN2.writeScaled(0.0f); }
}
...
void runController()
{
    ...
    if (xSemaphoreTake(g_mutex, pdMS_TO_TICKS(5)) == pdTRUE) {
        depth_target = g_depth_target; Kp = g_Kp; Ki = g_Ki; Kd = g_Kd; 
        K1 = g_K1; K2 = g_K2;
        K = g_K; tau1 = g_tau1; L = g_L; is_auto = g_is_auto; alpha_manual = g_alpha_manual;
        dirty = g_model_dirty; g_model_dirty = false;
    ...
    if (xSemaphoreTake(g_mutex, pdMS_TO_TICKS(5)) == pdTRUE) {
        g_setpoint = alpha_ref; g_tailboardangle = tailangle;
        xSemaphoreGive(g_mutex);
    }

    if (dirty) {
        g_model_wd.K = K; g_model_wd.tau1 = tau1; g_model_wd.L = L; g_model_wd.init(DT);
        g_model_nd.K = K; g_model_nd.tau1 = tau1; g_model_nd.L = 0.0f; g_model_nd.init(DT);
    }
    ...
    if (xSemaphoreTake(g_mutex, pdMS_TO_TICKS(5)) == pdTRUE) {
        g_liftingangle = liftingangle; g_e = e; g_de = de; g_s = s;
        g_u = u_out; g_u_eq = u_eq; g_u_sw = u_sw; g_u_sw_i = u_sw_int;
        xSemaphoreGive(g_mutex);
    }
}
...
void sensorReadTask(void *param)
{
    ...
    while (true) {
        ...
        g_lifting_raw_val = lifting_raw; g_tail_raw_val = tail_raw;
...
{
    TickType_t xLastWake = xTaskGetTickCount();
    while (true) { runController(); vTaskDelayUntil(&xLastWake, DT_TICKS); }
}
...
void serialTuningTask(void *param)
{
    ...
    while (true) {
        ...
        if (xSemaphoreTake(g_mutex, pdMS_TO_TICKS(5)) == pdTRUE) {
            float* v = msg.values;
            v[0]=g_lifting_raw_val; v[1]=g_liftingangle; v[2]=g_tail_raw_val; v[3]=g_tailboardangle; v[4]=g_sp_raw_val; v[5]=g_setpoint;
            v[6]=g_depth_target; v[7]=g_e; v[8]=g_de; v[9]=g_s; v[10]=g_u;
            v[11]=g_Kp; v[12]=g_Ki; v[13]=g_Kd; v[14]=g_K1; v[15]=g_K2;
            v[16]=g_K; v[17]=g_tau1; v[18]=g_L;
            v[19]=g_fc_lifting; v[20]=g_fc_tailboard; v[21]=g_omega_ref;
            v[22]=g_lift_offset; v[23]=g_tail_offset; v[24]=g_is_auto?1.0f:0.0f; v[25]=g_alpha_manual;
            v[26]=g_u_eq; v[27]=g_u_sw; v[28]=g_u_sw_i;
            xSemaphoreGive(g_mutex);
        }
            ...
            if (Serial.peek() == 0x55) {
                ...
                if (cmd->header == 0xAA55 && calculateChecksum((uint8_t *)&cmd->values, sizeof(cmd->values)) == cmd->checksum) {
                    if (xSemaphoreTake(g_mutex, pdMS_TO_TICKS(10)) == pdTRUE) {
                        float* cv = cmd->values;
                        g_depth_target=cv[0]; g_Kp=cv[1]; g_Ki=cv[2]; g_Kd=cv[3]; g_K1=cv[4]; g_K2=cv[5];
                        g_K=cv[6]; g_tau1=cv[7]; g_L=cv[8];
                        if (g_fc_lifting!=cv[9] || g_fc_tailboard!=cv[10] || g_omega_ref!=cv[11]) {
                            g_fc_lifting=cv[9]; g_fc_tailboard=cv[10]; g_omega_ref=cv[11];
                            filterLifting.init(g_fc_lifting, 860.0f); filterTailboard.init(g_fc_tailboard, 860.0f); refFilter.init(g_omega_ref, 1.0f);
                        }
                        g_lift_offset=cv[12]; g_tail_offset=cv[13]; g_is_auto=(cv[14]>0.5f); g_alpha_manual=cv[15];
                        g_model_dirty = true;
                        xSemaphoreGive(g_mutex);
                    }
                }
...
void setup()
{
    ...
    if (liftingsensor.begin(0x48)) {
        liftingsensor.setGain(GAIN_ONE); liftingsensor.setDataRate(RATE_ADS1115_860SPS);
        liftingsensor.startADCReading(ADS1X15_REG_CONFIG_MUX_DIFF_0_1, true);
    }
    if (tailboardsensor.begin(0x49)) {
        tailboardsensor.setGain(GAIN_ONE); tailboardsensor.setDataRate(RATE_ADS1115_860SPS);
    ...
    pwmIN1.attachPin(PIN_MOTOR_IN1, PWM_FREQ_HZ, PWM_BITS); pwmIN2.attachPin(PIN_MOTOR_IN2, PWM_FREQ_HZ, PWM_BITS);
    g_mutex = xSemaphoreCreateMutex();
    filterLifting.init(25.0f, 860.0f); filterTailboard.init(25.0f, 860.0f); refFilter.init(10.0f, 1.0f);
    g_model_wd.K = g_K; g_model_wd.tau1 = g_tau1; g_model_wd.L = g_L; g_model_wd.init(DT);
    g_model_nd.K = g_K; g_model_nd.tau1 = g_tau1; g_model_nd.L = 0.0f; g_model_nd.init(DT);
    g_model_dirty = false;
