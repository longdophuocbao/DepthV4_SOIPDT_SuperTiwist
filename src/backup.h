...
//  THAM SỐ ĐIỀU KHIỂN
// ────────────────────────────────────────────────────────────────────
volatile float g_Kp = 0.64f; // 1.4
volatile float g_Ki = 0.17f; // 0.5
volatile float g_Kd = 0.86f; // 0.57
volatile float g_K1 = 0.83f; // 0.47Gain của Super-Twisting SMC (√|s|·sign(s))
volatile float g_K2 = 0.35f; // 0.89Gain của Super-Twisting SMC (∫sign(s))

volatile float g_K = 0.076976f * 100.0f;
...
void runController()
{
    ...
    static float sp_raw_prev = sp_raw;
    static float e_int = 0.0f;
    static float u_sw_int = 0.0f;
    float delta_sp = sp_raw - sp_raw_prev;
    if (fabsf(delta_sp) > 0.5f) // step lớn hơn 2°
    {
        refFilter.x1 = sp_raw; // snap sp_ref về đích ngay lập tức
        refFilter.x2 = 0.0f;   // xoá vận tốc filter (không overshoot)
        e_int = 0.0f;          // xả Ki integral cũ
        u_sw_int = 0.0f;       // xả STA integral cũ  ← cần khai báo static bên ngoài
    }
    sp_raw_prev = sp_raw;
...
// ────────────────────────────────────────────────────────────────────
volatile float g_fc_lifting = 10.0f, g_fc_tailboard = 10.0f, g_omega_ref = 10.0f;
...
void setup()
{
    ...
    filterAlpha_actual_dot.init(2.0f, 500.0f);

    refFilter.init(15.0f, 1.0f);
    g_model_wd.K = g_K;
